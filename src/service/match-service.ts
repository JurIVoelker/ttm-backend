...
import { CREATE_MATCH_SCHEMA, UPDATE_MATCH_SCHEMA } from "../validation/match-schema";
import { HTTPException } from "hono/http-exception";
import { Availability } from "../prisma/generated";

export class MatchService {
  public async getMatchesByTeamSlug(teamSlug: string) {
    const targetTeam = await prisma.team.findUnique({ where: { slug: teamSlug } });

    if (!targetTeam) throw new HTTPException(404, { message: "Team not found", cause: `Team with slug "${teamSlug}" does not exist` });
    ...
    const matchesWithSortedLineup = matches.map((match) => ({
      ...match, lineup: match.lineup.sort((a, b) => {
  ...
  public async create({ data, teamSlug }: { data: z.infer<typeof CREATE_MATCH_SCHEMA>, teamSlug: string }) {
    const team = await prisma.team.findUnique({ where: { slug: teamSlug } })
    if (!team) throw new HTTPException(404, { message: "Team not found", cause: `Team with slug "${teamSlug}" does not exist` });
  ...
  public async update({ data, matchId }: { data: z.infer<typeof UPDATE_MATCH_SCHEMA>, matchId: string }) {
  ...
  public async matchBelongsToTeamAndPlayer({ matchId, teamSlug, playerId }: { matchId: string, teamSlug: string, playerId: string }) {
  ...
  public async vote({ availability, matchId, teamSlug, playerId }: { availability: Availability, matchId: string, teamSlug: string, playerId: string }) {
  ...
  public async setLineup({ matchId, playerIds }: { matchId: string, playerIds: string[] }) {
    const txResult = await prisma.$transaction(async (tx) => {
